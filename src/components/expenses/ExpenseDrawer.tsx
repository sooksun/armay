...
function ImageBox({ label, url }: { label: string; url: string | null }) {
  return (
    <div>
      <div style={{ fontSize: 11.5, color: "rgba(var(--text-rgb),0.55)", marginBottom: 5 }}>{label}</div>
      {url ? (
        // eslint-disable-next-line @next/next/no-img-element -- reason: in-memory object URL, not next/image-optimizable
        <img src={url} alt={label} style={{ display: "block", width: "100%", height: 110, objectFit: "cover", borderRadius: 12, border: "1px solid rgba(var(--surface-rgb),0.12)" }} />
      ) : (
        <div
          style={{
            height: 110,
    ...
    >
      ...
      <InfoSection title="ผู้รับผิดชอบ">
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
          <span style={{ color: "rgba(var(--text-rgb),0.55)" }}>ค่าใช้จ่ายนี้รับผิดชอบโดย</span>
