...
export async function getRentalDetail(id: number): Promise<RentalDetailDTO> {
  ...
  return {
    ...
    rentValue: decToNumber(c.rentAmount),
    depositValue: decToNumber(c.depositAmount),
    cleaningValue: decToNumber(c.cleaningFee),
    otherValue: decToNumber(c.otherFee),
    discountValue: decToNumber(c.discountAmount),
